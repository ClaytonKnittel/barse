...
fallback if this read would cross a page boundary, to prevent segfaulting were that to be the last page of the mmapped
file buffer) and mask off the bytes past the end of the
string[^str_mask].
...
table is 72 MB, which is about 3x larger than the L2 cache on my computer. The average probing distance for the magic
value found for tables this size is ~1.04, meaning ~95% of lookups find the key in the first bucket they search.
...
distance for the magic value found for tables this size is ~1.3, much higher than the single-threaded workload. This
shows that the memory working set size is much more constraining in a multithreaded environment than compute, relative
to single-threaded.

### String Comparison

Since we cannot assume the station names in the input file come from `data/weather_stations.csv`, we can't rely on the
uniqueness of a hash that we know does not collide on those weather station names. Therefore, we must always compare the
station names when looking them up in the table.

The fast path for string comparison is used for strings that are <= 32 bytes long and do not begin within 31 bytes of a
page boundary (again, for correctness in case this is the last page of the file buffer). Comparison is relatively
straightforward in this case - unaligned 32-byte load into a `ymm` register, mask off bytes past the end of the string
using the same trick we did for hashing, then simply a `vpxor` will set the zero flag if the comparison succeeded.

#### Redundant string loads in string hash and comparison

You may have noticed that we do an unaligned load of the string value from the file buffer and mask off bytes past the
end of the string twice, once for string hashing and once for string comparison. Could we simply load the data + mask
off foreign bytes once?

I tried this, and it turned out to be slower. The key reason comes from out-of-order execution and dependency chains,
and the fact that equivalent instructions on the 32-byte `ymm` registers have higher latency than on the 16-byte `xmm`
registers.

Here is a timeline view of the whole string hashing + table lookup + comparison algorithm, generated by `llvm-mca`:

                        0123456789          0123456789
    Index     0123456789          0123456789          0
    
    [0,0]     DeeER.    .    .    .    .    .    .    .   andl	$4095, %ebx
    [0,1]     D==eER    .    .    .    .    .    .    .   cmpq	$4080, %rbx
    [0,2]     D===eER   .    .    .    .    .    .    .   ja	.READ_ACROSS_PAGE_BOUNDARY
    [0,3]     DeeeeeeeER.    .    .    .    .    .    .   vmovdqu	(%r12), %xmm0
    [0,4]     DeE------R.    .    .    .    .    .    .   movl	$16, %eax
    [0,5]     D=eE-----R.    .    .    .    .    .    .   subq	%r15, %rax
    [0,6]     .DeE-----R.    .    .    .    .    .    .   movl	$0, %ecx
    [0,7]     .D=eE----R.    .    .    .    .    .    .   cmovaeq	%rax, %rcx
    [0,8]     .D=eE----R.    .    .    .    .    .    .   leaq	-120822(%rip), %rax
    [0,9]     .D==eeeeeeeeER .    .    .    .    .    .   vpand	(%rcx,%rax), %xmm0, %xmm0
    [0,10]    .D==========eER.    .    .    .    .    .   vpshufd	$238, %xmm0, %xmm1
    [0,11]    . D==========eER    .    .    .    .    .   vpxor	%xmm0, %xmm1, %xmm0
    [0,12]    . D===========eeeER .    .    .    .    .   vmovq	%xmm0, %rcx
    [0,13]    . DeE-------------R .    .    .    .    .   movabsq	$140754668228609, %rax
    [0,14]    . D==============eeeER   .    .    .    .   imulq	%rax, %rcx
    [0,15]    . D=================eER  .    .    .    .   shrq	$44, %rcx
    [0,16]    . D==================eER .    .    .    .   leaq	(%rcx,%rcx,8), %rdx
    [0,17]    .  D==================eER.    .    .    .   leaq	(%r8,%rdx,8), %rax
    [0,18]    .  D==================eeeeeER .    .    .   movl	68(%r8,%rdx,8), %edx
    [0,19]    .  D=======================eER.    .    .   cmpq	%rdx, %r15
    [0,20]    .  D========================eER    .    .   jne	.STRINGS_NOT_EQUAL
    [0,21]    .  D=======================eE-R    .    .   cmpl	$32, %edx
    [0,22]    .  D========================eER    .    .   ja	.SLOW_COMPARISON_FALLBACK
    [0,23]    .   DeE-----------------------R    .    .   cmpl	$4065, %ebx
    [0,24]    .   D=eE----------------------R    .    .   jae	.SLOW_COMPARISON_FALLBACK
    [0,25]    .   DeE-----------------------R    .    .   leaq	-104763(%rip), %rsi
    [0,26]    .   D=eE----------------------R    .    .   subq	%r15, %rsi
    [0,27]    .   DeeeeeeeeE----------------R    .    .   vmovdqu	(%r12), %ymm0
    [0,28]    .    D=eeeeeeeeeE-------------R    .    .   vpand	32(%rsi), %ymm0, %ymm0
    [0,29]    .    D=================eeeeeeeeeER .    .   vpxor	16(%rax), %ymm0, %ymm0
    [0,30]    .    D==========================eeeeeeER.   vptest	%ymm0, %ymm0
    [0,31]    .    .D===============================eER   jne	.STRINGS_NOT_EQUAL

The bottleneck of this algorithm is the dependency chain from loading 16 bytes of the string value -> hashing -> table
lookup -> load string value from table for comparison. The preparation of the string value from the file buffer for
comparison (i.e. 32-byte load + mask off foreign bytes, lines 27 - 28) happens out-of-order, while the hash is still
being calculated.  In other words, it is not on the critical path. This can happen speculatively, since the odds of
taking one of the branches before (the slow fallback) are low.
